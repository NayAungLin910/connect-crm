...
const ViewActivity = () => {
    const [lead, setLead] = useState(window.lead);
    ...
    const [by, setBy] = useState("anyone");
    ...
    const getActivities = () => {
        cusaxios
            .get(
                `/activity/view?startdate_activity=${startDate}&enddate_activity=${endDate}&search=${search}&time_sort=${timeSort}&type=${type}&done=${done}&user_type=${window.type}&user_id=${window.auth.id}&by=${by}&lead_id=${lead.id}&page=${curPage}`
            )
            .then((res) => {
    ...
    useEffect(() => {
        getActivities();
    }, [curPage, startDate, endDate, search, timeSort, type, done, by]);
            ...
            {loading ? (
                ...
                <div className="row">
                    ...
                    <div className="col-sm-5 my-2">
                        <div className="form-group">
                            <label htmlFor="timeSort-filter">Time</label>
                            <select
                        ...
                        <div className="row">
                            {activities.data.length == 0 ? (
                                ...
                                activities.data.map((a) => {
                                    ...
                                    return (
                                        <div className="col-sm-6" key={a.id}>
